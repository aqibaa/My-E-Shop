...
export default async function AccountPage() {
  ...
  if (!dbUser) {
    try {
      dbUser = await prisma.user.create({
        data: {
          id: userId, // Clerk ID hi Primary Key banegi
          name: `${clerkUser.firstName} ${clerkUser.lastName || ""}`,
          email: clerkUser.emailAddresses[0].emailAddress,
          role: "CUSTOMER",
          // image: clerkUser.imageUrl // Optional: Agar schema mein image field hai
        }
      });
    } catch (error) {
      // Race condition handle: Agar user create karte waqt koi error aaye 
      // (shayad parallel request se ban gaya ho), toh dobara find karo.
  ...
  try {
    ...
    // Agar error aaye to app crash na ho, empty orders dikhaye
    orders = [];
  }
